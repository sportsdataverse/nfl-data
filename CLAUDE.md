...
| `nfl_espn_build` (`espn_nfl_01..45_*_creation.py`) | `espn_nfl_pbp`, `espn_nfl_team_box`, `espn_nfl_player_box`, `espn_nfl_drives`, `espn_nfl_play_participants`, `espn_nfl_adv_{team,passing,rushing,receiving,defensive,turnover,drives,situational,defensive_players,specialists}`, the usage box per game `espn_nfl_adv_{player_usage,position_group_usage,tackles,position_group_tackles,team_usage,drive_scripting}` (shims 30-35; computed at BUILD time from each final's plays + participants by `sportsdataverse.football.usage_box`, so the installed sdv-py defines them) and their season leaderboards `espn_nfl_usage_{players,position_groups,tackles,position_group_tackles,teams,drive_scripting}` (shims 40-45; `aggregate_usage_box`, rates recomputed from summed counts) | 2002– | nfl-raw's **ESPN** library (`nfl/espn/raw\|plays`, read locally or over raw.githubusercontent.com) processed here with sdv-py `NFLPlayProcess` (offline: `espn_nfl_pbp(summary=)` + `play_participants_from_items`), one cached final per game (`.cache/nfl_espn_final`, stamped `<sdv-py version>+SCHEMA_REV`), then the same flatten/reshape rules as `cfbfastR-cfb-data`'s `espn_cfb_*`. `game_id` is the ESPN event id; `nflverse_game_id` / `season_type` ride along from the crosswalk. Tuesdays in season (`espn_nfl_cron.yml`). |
